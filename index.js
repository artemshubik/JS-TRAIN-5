...
function multiplyArray(array) {
  ...
  for (let element of array) {
    product *= element; // Множимо кожен елемент на загальний добуток
  }
...
function findMinMax([firstElement, ...restElements]) {
  ...
  let min = firstElement; // Ініціалізуємо змінну для мінімального значення, задаємо перше значення масиву
  let max = firstElement; // Ініціалізуємо змінну для максимального значення, задаємо перше значення масиву
...
function printStrings(array) {
  ...
  // Перетворюємо масив в рядок за допомогою toString()
  let string = array.toString();

  // Повторюємо рядок 2 рази
  let repeatedString = string + string;
...
function concatenateStrings(array, subString) {
  ...
  // Використовуємо метод toString() для перетворення масиву в рядок
  let concatenatedString = array.toString();

  // Перевіряємо, чи містить рядок підрядок
  let containsSubstring = concatenatedString.includes(subString);
...
function addTenToEach(array) {
  ...
  // Ініціалізуємо новий масив для зберігання результату
  let resultArray = [];

  // Перебираємо масив за допомогою циклу for-in
  for (let index in array) {
    // Додаємо до поточного елемента 10 і додаємо результат в новий масив
